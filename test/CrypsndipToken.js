...
contract('CrypsndipToken', function(accounts) {
    ...
    it('initializes the contract with correct values', function () {
        return CrypsndipToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Crypsndip Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'CRPS', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Crypsndip Token v1.0', 'has the correct standard');
        });
    })
